...
# --- ENHANCED PERSISTENCE STORAGE PATH FOR RAILWAY VOLUMES ---
# In Railway Dashboard -> Volume settings, mount a volume to "/data" and set PERSISTENT_STORAGE_DIR = /data
STORAGE_DIR = os.getenv("PERSISTENT_STORAGE_DIR", "/data")
if not os.path.exists(STORAGE_DIR):
    try:
        os.makedirs(STORAGE_DIR, exist_ok=True)
    except Exception as e:
        print(f"Warning: Could not create STORAGE_DIR {STORAGE_DIR}: {e}. Falling back to current directory.")
        STORAGE_DIR = "."
...
class WhisperCog(commands.Cog):
    ...
    async def on_ready(self):
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if env_lobby and env_lobby.isdigit():
                ...
                conn.execute("INSERT OR REPLACE INTO whisper_config (key, value) VALUES ('lobby_channel_id', ?)", (parsed_lobby_id,))
                lobby_channel_id = parsed_lobby_id

            env_servers = os.getenv("WHISPER_MONITORED_SERVER_IDS")
            if env_servers:
                for s_str in env_servers.split(","):
                    s_str = s_str.strip()
                    if s_str.isdigit():
                        conn.execute("INSERT OR IGNORE INTO whisper_server_logs (guild_id) VALUES (?)", (int(s_str),))
    ...
    async def setwhisper(self, ctx, channel: discord.TextChannel):
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            conn.commit()
        await async_save_backup_config("lobby_channel_id", channel.id)
    ...
    async def whisperserverset(self, ctx, server_id: int):
        with sqlite3.connect(DB_PATH) as conn:
            ...
            conn.commit()
        await async_save_backup_config("add_server", server_id)
