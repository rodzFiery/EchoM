...
@bot.command()
async def work(ctx): 
    # FIXED: Manual premium check to prevent lambda-related silent failures
    premium_cog = bot.get_cog("PremiumSystem")
    if premium_cog and hasattr(premium_cog, 'is_premium'):
        if not await premium_cog.is_premium().predicate(ctx):
            return
...
async def on_message(message):
    ...
    # FIXED: Security logic checked first, then process commands to prevent execution stall
    ...
    # FIXED: Commands are processed last
